...
module.exports = {
        ...
        "flowStats" : function flowStats_unpack(buffer, offset) {
                ...
                buffer.copy(flowStats.cookie, 0, offset, offset + 8);
                offset += 8;

                flowStats.packet_count = new Int64(buffer.readUInt32BE(offset, true),
                                                   buffer.readUInt32BE(offset + 4, true));
                offset += 8;

                flowStats.byte_count = new Int64(buffer.readUInt32BE(offset, true),
                                                 buffer.readUInt32BE(offset + 4, true));
                offset += 8;
        ...
        "groupStats" : function groupStats_unpack(buffer, offset) {
                ...
                groupStats.packet_count = new Int64(buffer.readUInt32BE(offset, true),
                                                    buffer.readUInt32BE(offset + 4, true));
                offset += 8;

                groupStats.byte_count = new Int64(buffer.readUInt32BE(offset, true),
                                                  buffer.readUInt32BE(offset + 4, true));
                offset += 8;
        ...
        "tableStats" : function tableStats_unpack(buffer, offset) {
                ...
                tableStats.lookup_count = new Int64(buffer.readUInt32BE(offset, true),
                                                    buffer.readUInt32BE(offset + 4, true));
                offset += 8;

                tableStats.matched_count = new Int64(buffer.readUInt32BE(offset, true),
                                                     buffer.readUInt32BE(offset + 4, true));
                offset += 8;
        ...
        "portStats" : function portStats_unpack(buffer, offset) {
                ...
                var rx_packets = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_packets.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_packets = rx_packets;
                }
                offset += 8;

                var tx_packets = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!tx_packets.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.tx_packets = tx_packets;
                }
                offset += 8;

                var rx_bytes = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_bytes.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_bytes = rx_bytes;
                }
                offset += 8;

                var tx_bytes = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!tx_bytes.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.tx_bytes = tx_bytes;
                }
                offset += 8;

                var rx_dropped = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_dropped.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_dropped = rx_dropped;
                }
                offset += 8;

                var tx_dropped = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!tx_dropped.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.tx_dropped = tx_dropped;
                }
                offset += 8;

                var rx_errors = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_errors.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_errors = rx_errors;
                }
                offset += 8;

                var tx_errors = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!tx_errors.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.tx_errors = tx_errors;
                }
                offset += 8;

                var rx_frame_err = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_frame_err.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_frame_err = rx_frame_err;
                }
                offset += 8;

                var rx_over_err = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_over_err.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_over_err = rx_over_err;
                }
                offset += 8;

                var rx_crc_err = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
                if (!rx_crc_err.buffer.equals(ofputil.UINT64_ALL)) {
                    portStats.rx_crc_err = rx_crc_err;
                }
                offset += 8;

                var collisions = new Int64(buffer.readUInt32BE(offset, true), buffer.readUInt32BE(offset + 4, true));
        ...
        "bucketCounter" : function bucketCounter(buffer, offset) {
                ...
                bucketCounter.packet_count = new Int64(buffer.readUInt32BE(offset, true),
                                                       buffer.readUInt32BE(offset + 4, true));
                offset += 8;

                bucketCounter.byte_count = new Int64(buffer.readUInt32BE(offset, true),
                                                     buffer.readUInt32BE(offset + 4, true));
                offset += 8;
        ...
        "match" : function match_unpack(buffer, offset) {
                ...
                switch (type) {
                    ...
                    case ofp.ofp_match_type.OFPMT_STANDARD: {
                        ...
                        buffer.copy(match.body.metadata, 0, offset, offset + 8);
                        match.body.metadata_mask = new Buffer(8);
                        buffer.copy(match.body.metadata_mask, 0, offset + 8, offset + 16);
                        offset += 16;

                        if (match.body.metadata_mask.equals(ofputil.UINT64_NONE)) {
                            match.body.metadata_mask = undefined;
                        }

                        if (match.body.metadata_mask.equals(ofputil.UINT64_ALL)) {
