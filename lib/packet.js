...
module.exports = {
        ...
        isEthNone : function(buf, offset) {
                            return buf.readUInt32BE(offset, true) === 0x00000000 &&
                                   buf.readUInt16BE(offset + 4, true) === 0x0000;
                        },

        isEthAll : function(buf, offset) {
                            return buf.readUInt32BE(offset, true) === 0xffffffff &&
                                   buf.readUInt16BE(offset + 4, true) === 0xffff;
                        },
        ...
        stringToIPv4 : function stringToIPv4(string, buffer, offset) {
                            ...
                            buffer.writeUInt8(parseInt(ip[1], 10), offset    , true); // i+1 because whole match is at loc. 0
                            buffer.writeUInt8(parseInt(ip[2], 10), offset + 1, true);
                            buffer.writeUInt8(parseInt(ip[3], 10), offset + 2, true);
                            buffer.writeUInt8(parseInt(ip[4], 10), offset + 3, true);
        }
};
