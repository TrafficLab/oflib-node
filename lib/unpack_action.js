...
module.exports = function action_unpack(buffer, offset) {
        ...
        if (buffer.length < offset + ofp.sizes.ofp_action_header) {
            return {
                "error" : util.format('Received action is too short (%d).', buffer.length),
        ...
        if (buffer.length < offset + len) {
            return {
                "error" : util.format('Received action has invalid length (set to %d, but only %d received).', len, buffer.length),
                "ofp_error_type" : 'OFPET_BAD_ACTION',
                "ofp_error_code" : 'OFPBAC_BAD_LEN'
            }
        }
