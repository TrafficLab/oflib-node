        ...
        switch (type) {
            ...
            case ofp.ofp_type.OFPT_ERROR: {
               ...
               offset += 4;
                ...
                if (len > ofp.sizes.ofp_error_msg) {
                    message.body.data = new Buffer(len - ofp.sizes.ofp_error_msg);
                    buffer.copy(message.body.data, 0, offset, offset + len - ofp.sizes.ofp_error_msg);
            ...
            case ofp.ofp_type.OFPT_ECHO_REQUEST: {
                ...
                if (len > ofp.sizes.ofp_header) {
                    message.body.data = new Buffer(len - ofp.sizes.ofp_header);
                    buffer.copy(message.body.data, 0, offset, offset + len - ofp.sizes.ofp_header);
            ...
            case ofp.ofp_type.OFPT_PACKET_IN: {
                ...
                if (len > ofp.sizes.ofp_packet_in + 2) { // pad = 2
                    ...
                    buffer.copy(message.body.data, 0, offset + 2, offset + 2 + dataLen);
                }
            ...
            case ofp.ofp_type.OFPT_FLOW_REMOVED: {
                ...
                buffer.copy(message.body.cookie, 0, offset, offset + 8);
                ...
                message.body.packet_count = new Int64(buffer.readUInt32BE(offset, true),
                                                      buffer.readUInt32BE(offset + 4, true));

                offset += 8;

                message.body.byte_count = new Int64(buffer.readUInt32BE(offset, true),
                                                    buffer.readUInt32BE(offset + 4, true));
                offset += 8;
            ...
            case ofp.ofp_type.OFPT_PACKET_OUT: {
                ...
                if (dataLen > 0) {
                    if (message.body.buffer_id == undefined) {
                        message.body.data = new Buffer(dataLen);
                        buffer.copy(message.body.data, 0, offset, offset + dataLen);
                    }
                    offset += dataLen;
                }
            ...
            case ofp.ofp_type.OFPT_FLOW_MOD: {
                ...
                buffer.copy(message.body.cookie_mask, 0, offset + 8, offset + 16);
                offset += 16;
            ...
            case ofp.ofp_type.OFPT_GROUP_MOD: {
                ...
                if (!ofputil.setEnum(message.body, 'type', buffer.readUInt8(offset, true), ofp.ofp_group_type_rev)) {
                    return {
            ...
            case ofp.ofp_type.OFPT_STATS_REQUEST: {
                ...
                var statType = buffer.readUInt16BE(offset, true);
                message.body.header = {};

                var flags = buffer.readUInt16BE(offset + 2, true);
                if (flags != 0) {
                    return {
                        "error" : util.format('Received %s message has invalid flags (%d).', message.header.type, flags),
                        "ofp_error" : {"type" : 'OFPET_BAD_REQUEST', "code" : 'OFPBRC_BAD_LEN'}
                ...
                switch (statType) {
                    case ofp.ofp_stats_types.OFPST_DESC: {
                        message.body.header.type = 'OFPST_DESC';
                        message.body.body = {};

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_FLOW: {
                        message.body.header.type = 'OFPST_FLOW';
                        message.body.body = {};

                        // NOTE: ofp_flow_stats_request does contain an ofp_match
                        ...
                        // TODO sanity check?
                        message.body.body.table_id = buffer.readUInt8(offset, true);
                        if (message.body.body.table_id == ofp.ofp_table.OFPTT_ALL) {
                            message.body.body.table_id = 'OFPTT_ALL';
                        }
                        offset += 4; /* + pad */

                        ofputil.setIfNotEq(message.body.body, 'out_port', buffer.readUInt32BE(offset, true), ofp.ofp_port_no.OFPP_ANY); // TODO sanity check?
                        ofputil.setIfNotEq(message.body.body, 'out_group', buffer.readUInt32BE(offset + 4, true), ofp.ofp_group.OFPG_ANY); // TODO sanity check?
                        offset += 12;

                        message.body.body.cookie = new Buffer(8);
                        buffer.copy(message.body.body.cookie, 0, offset, offset + 8);
                        message.body.body.cookie_mask = new Buffer(8);
                        buffer.copy(message.body.body.cookie_mask, 0, offset + 8, offset + 16);
                        offset += 16;
                        ...
                        message.body.body.match = unpack.match;
                        offset = unpack.offset;

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_AGGREGATE: {
                        message.body.header.type = 'OFPST_AGGREGATE';
                        message.body.body = {};
                    ...
                    case ofp.ofp_stats_types.OFPST_TABLE: {
                        message.body.header.type = 'OFPST_TABLE';
                        message.body.body = {};

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_PORT: {
                        message.body.header.type = 'OFPST_PORT';
                        message.body.body = {};

                        ofputil.setIfNotEq(message.body.body, 'port_no', buffer.readUInt32BE(offset, true), ofp.ofp_port_no.OFPP_ANY); // TODO sanity check?
                        offset += 8; /* + pad */

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_QUEUE: {
                        message.body.header.type = 'OFPST_QUEUE';
                        message.body.body = {};

                        ofputil.setIfNotEq(message.body.body, 'port_no', buffer.readUInt32BE(offset, true), ofp.ofp_port_no.OFPP_ANY); // TODO sanity check?
                        ofputil.setIfNotEq(message.body.body, 'queue_id', buffer.readUInt32BE(offset + 4, true), ofp.OFPQ_ANY); // TODO sanity check?
                        offset += 8; /* + pad */

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_GROUP: {
                        message.body.header.type = 'OFPST_GROUP';
                        message.body.body = {};

                        ofputil.setIfNotEq(message.body.body, 'group_id', buffer.readUInt32BE(offset, true), ofp.ofp_group.OFPG_ANY); // TODO sanity check?
                        offset += 8; /* + pad */
                    ...
                    case ofp.ofp_stats_types.OFPST_GROUP_DESC: {
                        message.body.header.type = 'OFPST_GROUP_DESC';
                        message.body.body = {};

                        return {"message" : message, "offset" : offset};
                    }

                    case ofp.ofp_stats_types.OFPST_EXPERIMENTER: {
                        // TODO: Implement Experimenter
                        message.body.header.type = 'OFPST_EXPERIMENTER';
                        message.body.body = {};

                        offset += len - ofp.sizes.ofp_stats_request;
            ...
            case ofp.ofp_type.OFPT_STATS_REPLY: {
                ...
                switch (statType) {
                    case ofp.ofp_stats_types.OFPST_DESC: {
                        ...
                        message.body.body.mfr_desc = buffer.toString('utf8', offset, offset + ofp.DESC_STR_LEN);
                        message.body.body.mfr_desc = message.body.body.mfr_desc.substr(0, message.body.body.mfr_desc.indexOf('\0'));
                        offset += ofp.DESC_STR_LEN;

                        message.body.body.hw_desc = buffer.toString('utf8', offset, offset + ofp.DESC_STR_LEN);
                        message.body.body.hw_desc = message.body.body.hw_desc.substr(0, message.body.body.hw_desc.indexOf('\0'));
                        offset += ofp.DESC_STR_LEN;

                        message.body.body.sw_desc = buffer.toString('utf8', offset, offset + ofp.DESC_STR_LEN);
                        message.body.body.sw_desc = message.body.body.sw_desc.substr(0, message.body.body.sw_desc.indexOf('\0'));
                        offset += ofp.DESC_STR_LEN;

                        message.body.body.serial_num = buffer.toString('utf8', offset, offset + ofp.SERIAL_NUM_LEN);
                        message.body.body.serial_num = message.body.body.serial_num.substr(0, message.body.body.serial_num.indexOf('\0'));
                        offset += ofp.SERIAL_NUM_LEN;

                        message.body.body.dp_desc = buffer.toString('utf8', offset, offset + ofp.DESC_STR_LEN);
                        message.body.body.dp_desc = message.body.body.dp_desc.substr(0, message.body.body.dp_desc.indexOf('\0'));
                        offset += ofp.DESC_STR_LEN;
                    ...
                    case ofp.ofp_stats_types.OFPST_FLOW: {
                        ...
                        message.body.body.stats = [];
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.flowStats(buffer, offset);
                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['flow-stats']);
                            offset = unpack.offset;
                        }

                        if (offset != statsEnd) {
                            return {
                                "error" : util.format('The %s received message contained extra bytes (%d).', message.header.type, (statsEnd - offset))
                            }
                        }
                    ...
                    case ofp.ofp_stats_types.OFPST_AGGREGATE: {
                        ...
                        message.body.body.packet_count = new Int64(buffer.readUInt32BE(offset, true),
                                                                   buffer.readUInt32BE(offset + 4, true));
                        offset += 8;

                        message.body.body.byte_count = new Int64(buffer.readUInt32BE(offset, true),
                                                                 buffer.readUInt32BE(offset + 4, true));
                        offset += 8;

                        message.body.body.flow_count = buffer.readUInt32BE(offset, true);
                        offset += 8; /* + pad */
                    ...
                    case ofp.ofp_stats_types.OFPST_TABLE: {
                        ...
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.tableStats(buffer, offset);
                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['table-stats']);
                            offset = unpack.offset;
                        }
                    ...
                    case ofp.ofp_stats_types.OFPST_PORT: {
                        ...
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.portStats(buffer, offset);
                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['port-stats']);
                            offset = unpack.offset;
                        }
                    ...
                    case ofp.ofp_stats_types.OFPST_QUEUE: {
                        ...
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.queueStats(buffer, offset);

                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['queue-stats']);
                            offset = unpack.offset;
                        }
                    ...
                    case ofp.ofp_stats_types.OFPST_GROUP: {
                        ...
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.groupStats(buffer, offset);

                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['group-stats']);
                            offset = unpack.offset;
                        }
                    ...
                    case ofp.ofp_stats_types.OFPST_GROUP_DESC: {
                        ...
                        var statsEnd = offset + len - ofp.sizes.ofp_stats_reply;
                        while (offset < statsEnd) {
                            var unpack = unpackStruct.groupDescStats(buffer, offset);

                            if ('error' in unpack) {
                                return unpack;
                            }
                            message.body.body.stats.push(unpack['group-desc-stats']);
                            offset = unpack.offset;
                        }
            ...
            case ofp.ofp_type.OFPT_QUEUE_GET_CONFIG_REQUEST: {
                ...
                message.body.port = buffer.readUInt32BE(offset, true);
                if (message.body.port > ofp.ofp_port_no.OFPP_MAX && message.body.port != ofp.ofp_port_no.OFPP_ALL) {
                    return {
                        "error" : util.format('Received %s message has invalid port (%d).', message.header.type, message.body.port),
                        "ofp_error" : {"type" : 'OFPET_BAD_REQUEST', "code" : 'OFPBRC_BAD_LEN'}
                    }
                }

                if (message.body.port == ofp.ofp_port_no.OFPP_ALL) {
            ...
            case ofp.ofp_type.OFPT_QUEUE_GET_CONFIG_REPLY: {
                ...
                message.body.port = buffer.readUInt32BE(offset, true);
                if (message.body.port > ofp.ofp_port_no.OFPP_MAX) {
                    return {
                        "error" : util.format('Received %s message has invalid port (%d).', message.header.type, message.body.port),
                    }
                }
                ...
                message.body.queues = [];
                var queuesEnd = offset + len - ofp.sizes.ofp_queue_get_config_reply;
                while (offset < queuesEnd) {
                    var unpack = unpackStruct.packetQueue(buffer, offset);
                    if ('error' in unpack) {
                        return unpack;
                    }
                ...
                if (offset != queuesEnd) {
                    return {
                        "error" : util.format('The received %s contained extra bytes (%d).', message.header.type, (queuesEnd - offset)),
                    }
                }
