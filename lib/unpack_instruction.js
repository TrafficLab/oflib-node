...
module.exports = function instruction_unpack(buffer, offset) {
        ...
        if (buffer.length < offset + ofp.sizes.ofp_instruction) {
            return {
                "error" : util.format('Received instruction is too short (%d).', buffer.length),
        ...
        if (buffer.length < offset + len) {
            return {
                "error" : util.format('Received instruction has invalid length (set to %d, but only %d received).', len, buffer.length),
                "ofp_error_type" : 'OFPET_BAD_ACTION',
                "ofp_error_code" : 'OFPBAC_BAD_LEN'
            }
        }
        ...
        switch (type) {
            case ofp.ofp_instruction_type.OFPIT_GOTO_TABLE: {
                ...
                if (instruction.table_id > ofp.ofp_tables.OFPTT_MAX) {
                    return {
